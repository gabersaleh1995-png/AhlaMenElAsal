...
class AuthViewModel : ViewModel() {
            ...
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    val userUpdates = hashMapOf(
                        ...
                        "email" to (user?.email ?: ""),
                        "isAdmin" to false // الوضع الافتراضي ليس أدمن
                    )
                    ...
                    isLoading.value = false
                    errorMessage.value = "فشل الدخول بجوجل: ${task.exception?.localizedMessage}"
                }
            }
    }
    ...
    fun resetPassword(email: String, onComplete: (Boolean, String?) -> Unit) {
        if (email.isBlank()) {
            onComplete(false, "يرجى كتابة البريد الإلكتروني أولاً")
            return
        }
        isLoading.value = true
        auth.sendPasswordResetEmail(email.trim())
            .addOnCompleteListener { task ->
                isLoading.value = false
                if (task.isSuccessful) {
                    onComplete(true, "تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني")
                } else {
                    onComplete(false, task.exception?.localizedMessage ?: "حدث خطأ ما")
                }
            }
    }
}
